/**
 * DRAFT — Products Overview / Catalog Page
 *
 * Replaces the e-commerce-style Products.tsx with a B2B + DTC oriented
 * catalog tailored for parking AC:
 *   - Quick filter by vehicle / use case (not by price)
 *   - Product cards focused on BTU, voltage, install type, "best for"
 *   - Spec comparison table at the bottom
 *
 * Status: DRAFT for preview only. Live route /products/ is unchanged.
 * Preview at: /drafts/products
 */
...
/* ── Product catalog (single source of truth for this draft) ─────────── */
...
interface CatalogItem {
  ...
  bestFor: string;
  isNew?: boolean;
  status: "shipping" | "limited" | "preorder";
}
...
export default function DraftProductsOverview() {
  ...
  return (
    <PageLayout>
      {/* Top switcher so users can also jump straight to a product */}
      <DraftProductLineSwitcher previewMode />
      ...
      {/* Filter chips */}
      <section className="sticky z-20 bg-white border-b border-border" style={{ top: 64 + 56 /* navbar + switcher */ }}>
